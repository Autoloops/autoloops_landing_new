...
import { Brain, Target, LineChart, Sparkles, AlertCircle, RefreshCw, Layers, Zap } from 'lucide-react';

const FeatureCard = ({ icon: Icon, title, description, delay = 0, large = false }) => {
  return (
    <motion.div
...
const ProblemSolutionCard = ({ problem, solution, delay }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 30 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      transition={{ duration: 0.6, delay }}
      className="bg-slate-50/50 rounded-3xl p-8 border border-slate-200/60 hover:bg-white hover:border-teal-100 transition-all duration-300"
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <h4 className="font-semibold text-slate-900 mb-2">How Autoloops Solves It</h4>
...
const Features = () => {
  const features = [
    ...
    },
    {
      ...
      description: 'Continuous monitoring and adjustment of bids, budgets, and placements. Our agents work 24/7 to ensure every dollar drives maximum return, adapting to market changes instantly.',
      large: true,
    },
    {
      icon: Layers,
  ...
  const problemsSolutions = [
    {
      problem: 'Manually reviewing hundreds of ad creatives is time-consuming and you often miss subtle patterns that could scale performance.',
      solution: 'Our AI agents analyze every creative element automatically, surfacing winning patterns and recommending iterations backed by your performance data.',
    },
    {
      problem: 'Ad fatigue kills campaign performance, but catching it early requires constant monitoring that most teams simply don\'t have time for.',
      solution: 'Autoloops tracks creative fatigue across all campaigns in real-time, automatically pausing underperformers and reallocating budget to winners.',
    },
    {
      problem: 'Your team spends hours building reports instead of optimizing campaigns. By the time you spot issues, you\'ve already burned budget.',
      solution: 'Lightning-fast dashboards with AI-powered insights show exactly what\'s working. Get proactive recommendations, not retrospective reports.',
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-center mb-4">
            Stop Fighting Ad Platform Complexity
          </h2>
          <p className="text-lg text-slate-600 text-center mb-12 max-w-2xl mx-auto">
            Common pain points we solve for growth teams every day
          </p>
        </motion.div>
